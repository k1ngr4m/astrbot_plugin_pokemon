...
class PokeballStrategy:
    ...
    @staticmethod
    def get_master_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository') -> float:
        """大师球 - 255倍数（确保捕捉成功）"""
        return 255.0

    @staticmethod
    def get_ultra_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository') -> float:
        """高级球 - 2倍"""
        return 2.0

    @staticmethod
    def get_great_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository') -> float:
        """超级球 - 1.5倍"""
        return 1.5

    @staticmethod
    def get_poke_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository') -> float:
        """普通精灵球 - 1倍"""
        return 1.0

    @staticmethod
    def get_safari_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository') -> float:
        """狩猎球 - 1.5倍（特定区域使用，这里简化为固定1.5倍）"""
        return 1.5

    @staticmethod
    def get_net_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository') -> float:
    ...
    @staticmethod
    def get_dive_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository') -> float:
        """潜水球 - 3.5倍针对水上/钓鱼时遇到的宝可梦，1倍其他"""
        # 这里先按3.5倍处理，可以根据具体情境调整
        return 3.5

    @staticmethod
    def get_nest_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository') -> float:
    ...
    @staticmethod
    def get_repeat_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository', user_id: str, user_pokemon_repo: 'AbstractUserPokemonRepository') -> float:
    ...
    @staticmethod
    def get_timer_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository') -> float:
        """计时球 - 捕捉率随回合增加，最高4倍"""
        # 这里先按1倍处理，实际应用中可能需要根据战斗回合数调整
        return 1.0

    @staticmethod
    def get_luxury_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository') -> float:
        """豪华球 - 1倍（基础捕捉率不变）"""
        return 1.0

    @staticmethod
    def get_premier_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository') -> float:
        """纪念球 - 1倍"""
        return 1.0

    @staticmethod
    def get_dusk_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository') -> float:
    ...
    @staticmethod
    def get_heal_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository') -> float:
        """治愈球 - 1倍（基础捕捉率不变）"""
        return 1.0

    @staticmethod
    def get_quick_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository') -> float:
        """先机球 - 首回合4倍，其他回合1倍"""
        # 这里先按4倍处理，实际应用中可能需要判断是否为战斗首回合
        return 4.0

    @staticmethod
    def get_cherish_ball_multiplier(wild_pokemon: 'WildPokemonInfo', pokemon_repo: 'AbstractPokemonRepository') -> float:
        """贵重球 - 1倍"""
        return 1.0
